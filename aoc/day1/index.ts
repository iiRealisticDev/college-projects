...
// replace "one" to "nine" in words with the number it represents
function replace() {
  input.replace(/one/g, "1");
  input.replace(/two/g, "2");
  input.replace(/three/g, "3");
  input.replace(/four/g, "4");
  input.replace(/five/g, "5");
  input.replace(/six/g, "6");
  input.replace(/seven/g, "7");
  input.replace(/eight/g, "8");
  input.replace(/nine/g, "9");
}

// split array by new line
// get first and last number in each array
function getNums() {
  let arr = input.split("\n");

  for (const line in arr) {
    let nums = arr[line].split(/[^0-9]/).join("").split("");
    if (nums.length > 0) {
